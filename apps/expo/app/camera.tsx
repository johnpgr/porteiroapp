...
export default function CameraScreen() {
  ...
  const handleTakePhoto = async () => {
    ...
    try {
      ...
      if (photo?.uri) {
        ...
        console.log('🎯 [CameraModal] Photo ready for upload');

        // TODO: Upload logic will be handled by parent screen via callback or hook
        // For now, just show the photo preview
      }
    } catch (error) {
  ...
  const handleConfirm = () => {
    if (photoUri) {
      // TODO: Pass the photo back to the parent screen
      // This could be done via navigation params or a callback
      router.back();
    }
  };
  ...
  if (!permission.granted) {
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          <Text style={styles.title}>{params.title || 'Tirar Foto'}</Text>
          <TouchableOpacity onPress={handleClose} style={styles.closeButton}>
            <Ionicons name="close" size={24} color="#666" />
          </TouchableOpacity>
        </View>
