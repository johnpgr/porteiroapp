...
export default function PorteiroAutorizacoesScreen() {
  ...
  if (!isRegularUser(user)) {
    return (
      <View style={styles.centered}>
        <Text style={styles.message}>Esta tela é apenas para porteiros.</Text>
      </View>
    );
  }
  ...
  return <AutorizacoesView buildingId={buildingId} user={user} />;
}
...
const AutorizacoesView: React.FC<AutorizacoesViewProps> = ({
  buildingId,
  user,
  filter = 'all',
  ...
  const [activeSection, setActiveSection] = useState<'visitors' | 'preauthorized' | 'apartment'>(
    'visitors'
  );
          ...
          logsData?.map((log) => {
            const mapped = {
              ...
              resident_response_by_name: log.resident_response_by
                ...
                : null,
              authorized_by_name: log.authorized_by
                ? authorizedNames[log.authorized_by] || null
                : null,
            };
  ...
  const searchVisitorsByApartment = async (aptNumber: string) => {
    ...
    try {
      ...
      const processedVisitors: ActivityEntry[] = (visitors || []).map(
        (visit: VisitorWithApartment) => {
          const isApproved = visit.status === 'aprovado';
          const isPending = visit.status === 'pendente';
          const visitorName = visit.name || 'Visitante';
