  ...
  const checkFirstLoginStatus = useCallback(async () => {
    ...
    try {
      ...
      if (hasCpf && !hasFirstLoginCompleted) {
        console.log(
          '� DEBUG useFirstLogin - CPF existe mas first_login_completed é false, corrigindo...'
        );
        try {
          await supabase
            .from('profiles')
      ...
      if (updateError) {
        ...
        if (updateError.code === '23505') {
          return {
            success: false,
            error:
              'Dados duplicados encontrados. Verifique se as informações não estão já cadastradas no sistema.',
          };
        }
