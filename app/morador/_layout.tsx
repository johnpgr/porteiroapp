...
export default function MoradorLayout() {
    ...
    async (participant: Record<string, any> | null) => {
      ...
      if (incomingStatuses.has(status)) {
        ...
        const minimalCallData: IntercomCallData = {
          ...
          doormanName:
            typeof participant.doorman_name === 'string' && participant.doorman_name.length > 0
              ...
              : 'Porteiro',
          apartmentNumber:
            typeof participant.apartment_number === 'string' ? participant.apartment_number : undefined,
          buildingId: typeof participant.building_id === 'string' ? participant.building_id : undefined,
          buildingName:
        ...
        if (!statusResponse.success || !statusResponse.data?.call) {
          console.warn('⚠️ [MoradorLayout] Não foi possível obter detalhes da chamada:', statusResponse.error);
          return;
        }
        ...
        const enrichedCallData: IntercomCallData = {
          ...
          doormanName:
            ...
            minimalCallData.doormanName ?? 'Porteiro',
          buildingId:
            callInfo.buildingId ??
            (typeof callInfoAny.building_id === 'string' ? callInfoAny.building_id : null) ??
            minimalCallData.buildingId ?? null,
          buildingName:
            callInfo.buildingName ??
            (typeof callInfoAny.building_name === 'string' ? callInfoAny.building_name : undefined) ??
  ...
  useEffect(() => {
    ...
    responseListener.current = Notifications.addNotificationResponseReceivedListener((response) => {
      ...
      console.log('📞 [MoradorLayout] Resposta à notificação de chamada:', response.actionIdentifier);
