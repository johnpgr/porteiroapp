...
async function makeRequest<T>(path: string, options: RequestInit = {}): Promise<ApiResponse<T>> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('❌ [INTERCOM_API] Request failed:', { url, status: response.status, errorPayload });
      return {
        success: false,
        error: errorMessage,
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Erro de rede ao contatar serviço do interfone',
    };
  }
}
...
export async function initiateIntercomCall(
  ...
  const response = await makeRequest<StartCallResponse>(
    '/api/calls/start',
    {
      method: 'POST',
  ...
  const { call, participants = [], apartment = null, doorman = null, notificationsSent = 0, message } =
    response.data;
...
export async function rejectIntercomCall(
  ...
  const endpoint = reason === 'declined' ? `/api/calls/${callId}/decline` : `/api/calls/${callId}/end`;
